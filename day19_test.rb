...
class Day19Test < Test::Unit::TestCase
    ...
    def test_match_with_vectors
        scanners = parse("inputs/day19_test.txt")
        adjusted_scanner, scanner_loc = match_with_vectors(scanners[0], scanners[1])
        # best = vectors.keys.sort_by { |k| vectors[k] }
        # best.each do |b|
        #     puts "#{b.inspect} => #{vectors[b]}"
        # end
        # puts rotation
        puts (scanner_loc).inspect
        ...
        puts "-----"
        puts adjusted_scanner.transformed_beacons_prime.sort_by { |x| x[0] }.map { |v| v.inspect }
        puts "-----"
